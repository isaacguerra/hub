class VisitasController < ApplicationController
  ...
  def create
    ...
    if @visita.save
      redirect_to visitas_path, notice: "Visita registrada com sucesso."
    else
  ...
  def update
    if @visita.update(visita_params)
      redirect_to visitas_path, notice: "Visita atualizada com sucesso."
    else
  ...
  def destroy
    @visita.destroy
    redirect_to visitas_path, notice: "Visita excluída com sucesso."
  end
  ...
  def authorize_create
    unless Current.apoiador.pode_coordenar? || Current.apoiador.lider?
      redirect_to visitas_path, alert: "Você não tem permissão para registrar visitas."
    end
  end
  ...
  def authorize_manage
    ...
    unless can_manage
      redirect_to visitas_path, alert: "Você não tem permissão para gerenciar esta visita."
    end
  end
end
